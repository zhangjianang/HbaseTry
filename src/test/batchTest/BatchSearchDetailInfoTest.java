...
import com.company.HbaseStudy.ReadWriteFile;
import com.company.SearchDetailInfo;
import com.company.batchDetail.BatchSearchDetailInfo;
 ...
 */
public class BatchSearchDetailInfoTest {

    private BatchSearchDetailInfo bSearchDetailInfo;
    private SearchDetailInfo singleSearchDetail;
    @Before
    public void setUp() throws Exception {
        bSearchDetailInfo=new BatchSearchDetailInfo();
        singleSearchDetail=new SearchDetailInfo();
    }

    @Test
    public void getDetailInfoByCompanyId() throws Exception {
        Map<String, Object> detailInfoMap = bSearchDetailInfo.getDetailInfoByCompanyId("1", "10");
//        for(Map.Entry<String,Object> per:detailInfoMap.entrySet() ){
//            per.getKey();
//            per.getValue();
//        }

//        Map<String,String> batchResMap=new HashMap<String,String>();
//        Map<String,String> singleResMap=new HashMap<String,String>();
//        for(int i=1;i<100;i++){
//            String getRes= JSONObject.toJSONString(detailInfoMap.get(i+""));
//            batchResMap.put(i+"",getRes);
//            String getSingle =JSONObject.toJSONString(singleSearchDetail.getDetailInfoByCompanyId(i+""));
//            singleResMap.put(i+"",getSingle);
//            compareDiff(getRes,getSingle);
//        }

        String getSingle =JSONObject.toJSONString(singleSearchDetail.getDetailInfoByCompanyId(""));
       String fileGet = ReadWriteFile.readFileByLines(BatchSearchDetailInfoTest.class.getClassLoader().getResource("data/batchDetail.json").getPath());
        compareDiff(getSingle,fileGet);
    }

    public void compareDiff(String first,String second) {
        if (first != null && second != null) {
            JsonParser parser = new JsonParser();
            JsonObject obj = (JsonObject) parser.parse(first);
            System.out.println(obj);

            JsonParser parser1 = new JsonParser();
            JsonObject obj1 = (JsonObject) parser1.parse(second);
